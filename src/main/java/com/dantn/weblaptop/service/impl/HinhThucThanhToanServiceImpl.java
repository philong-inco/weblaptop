...
public class HinhThucThanhToanServiceImpl implements HinhThucThanhToanService {
    ...
    public String payWithVNPAYOnline2(CreateHoaDonClientRequest createHoaDonClientRequest, HoaDon hoaDon, HttpServletRequest request) {
        ...
        vnpParamsMap.put("vnp_ReturnUrl", this.returnUrlOnline);
        vnpParamsMap.put("vnp_Amount", String.valueOf(amount));
        vnpParamsMap.put("vnp_TxnRef",hoaDon.getMa() + "|" + VNPayUtil.getRandomNumber(8)+"|"+hoaDon.getSdt());
    ...
    @Override
    public String payWithVNPAYOnlineAdmin(String maHoaDon, HttpServletRequest request) {
        BigDecimal amount = BigDecimal.valueOf(Long.parseLong(request.getParameter("amount"))).multiply(BigDecimal.valueOf(100L));
        String bankCode = request.getParameter("bankCode");
        Map<String, String> vnpParamsMap = vnPayConfig.getVNPayConfig();
        vnpParamsMap.put("vnp_ReturnUrl", this.returnUrl+maHoaDon);
        vnpParamsMap.put("vnp_Amount", String.valueOf(amount));
        vnpParamsMap.put("vnp_TxnRef",maHoaDon + "|" + VNPayUtil.getRandomNumber(8));
